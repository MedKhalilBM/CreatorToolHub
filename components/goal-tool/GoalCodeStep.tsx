...
import { generateGoalHTML, generateGoalCSS, generateGoalJS, generateGoalData } from '../../services/goalCodeGenerator';

interface GoalCodeStepProps {
  settings: GoalSettings;
}

type CodeType = 'html' | 'css' | 'js' | 'data';

const GoalCodeStep: React.FC<GoalCodeStepProps> = ({ settings }) => {
  ...
  const codes = useMemo(() => ({
    html: generateGoalHTML(settings),
    css: generateGoalCSS(settings),
    js: generateGoalJS(settings),
    data: generateGoalData(settings),
  }), [settings]);
      ...
      <div className="flex-1 bg-black border border-tiger-border flex flex-col shadow-[0_0_30px_rgba(0,0,0,0.5)]">
        <div className="flex border-b border-tiger-border bg-tiger-surface overflow-x-auto scrollbar-hide">
          {(Object.keys(codes) as CodeType[]).map(tab => (
            ...
            >
              {activeTab === tab && <div className="absolute top-0 left-0 right-0 h-[2px] bg-tiger-yellow"></div>}
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="relative flex-1 bg-black overflow-hidden group">
              ...
              <button
                ...
                className={`
                    ...
                    ${copiedStates[activeTab] 
                        ? 'bg-tiger-yellow text-black border-tiger-yellow' 
                        : 'bg-black text-tiger-yellow border-tiger-yellow hover:bg-tiger-yellow hover:text-black'}
                `}
              >
          ...
          <pre className="h-full p-4 md:p-6 overflow-auto text-[10px] md:text-xs font-mono leading-relaxed text-gray-300 scrollbar-thin selection:bg-tiger-yellow selection:text-black">
