...
import { generateTimerHTML, generateTimerCSS, generateTimerJS, generateTimerData } from '../../services/timerCodeGenerator';

interface TimerCodeStepProps {
  settings: TimerSettings;
}

type CodeType = 'html' | 'css' | 'js' | 'data';

const TimerCodeStep: React.FC<TimerCodeStepProps> = ({ settings }) => {
  ...
  const codes = useMemo(() => ({
    html: generateTimerHTML(settings),
    css: generateTimerCSS(settings),
    js: generateTimerJS(settings),
    data: generateTimerData(settings),
  }), [settings]);
      ...
      <div className="flex-1 bg-black border border-tiger-border flex flex-col shadow-[0_0_30px_rgba(0,0,0,0.5)]">
        <div className="flex border-b border-tiger-border bg-tiger-surface overflow-x-auto scrollbar-hide">
          {(Object.keys(codes) as CodeType[]).map(tab => (
            ...
            >
              {activeTab === tab && <div className="absolute top-0 left-0 right-0 h-[2px] bg-tiger-purple"></div>}
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="relative flex-1 bg-black overflow-hidden group">
              ...
              <button
                ...
                className={`
                    ...
                    ${copiedStates[activeTab] 
                        ? 'bg-tiger-purple text-black border-tiger-purple' 
                        : 'bg-black text-tiger-purple border-tiger-purple hover:bg-tiger-purple hover:text-black'}
                `}
              >
          ...
          <pre className="h-full p-4 md:p-6 overflow-auto text-[10px] md:text-xs font-mono leading-relaxed text-gray-300 scrollbar-thin selection:bg-tiger-purple selection:text-black">
